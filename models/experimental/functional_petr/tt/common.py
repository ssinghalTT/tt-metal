...
class Conv_with_split:
    ...
    def __call__(self, device, input_tensor):
        ...
        for i in range(self.split_factor):
            ...
            if i == 0:
                torch_output_tensor = tt_output_tensor_on_device
            else:
                torch_output_tensor = ttnn.add(torch_output_tensor, tt_output_tensor_on_device)

        # output_tensor = ttnn.from_torch(torch_output_tensor, dtype=ttnn.bfloat16, device=device)
        output_tensor = ttnn.to_layout(torch_output_tensor, layout=ttnn.ROW_MAJOR_LAYOUT)
