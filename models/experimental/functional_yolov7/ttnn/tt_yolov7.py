...
class ttnn_detect:
    ...
    def _make_grid(self, nx=20, ny=20):
        x_coords = ttnn.arange(0, nx, 1, device=self.device)
        x_coords = ttnn.reshape(x_coords, (1, nx))
        x_coords = ttnn.expand(x_coords, [ny, nx])
        y_coords = ttnn.arange(0, ny, 1, device=self.device)
...
class ttnn_yolov7:
    def __init__(self, device, parameters) -> None:
        ...
        self.conv32 = Conv([1, 20, 20, 1024], (1, 1, 1, 1, 0, 0, 1, 1), parameters["39"], enable_act_double_buffer=True)
        self.conv33 = Conv(
            [1, 40, 40, 1024], (1, 1, 1, 1, 0, 0, 1, 1), parameters["40"], act_block_h=64, enable_act_double_buffer=True
        )
        self.conv34 = Conv(
            [1, 40, 40, 512],
            (3, 3, 2, 2, 1, 1, 1, 1),
    ...
    def __call__(self, input_tensor):
        ...
        conv42 = ttnn.upsample(conv42, 2, memory_config=ttnn.L1_MEMORY_CONFIG)  # PCC = 0.8895474558822815
        ...
        conv51 = ttnn.upsample(conv51, 2, memory_config=ttnn.L1_MEMORY_CONFIG)  # PCC = 0.9860659406120288
        ...
        conv62 = ttnn.concat(
            [conv62, conv60, conv50], dim=3, memory_config=ttnn.L1_MEMORY_CONFIG
