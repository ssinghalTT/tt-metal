...
def ttnn_vgg11(
    ...
    for itr, v in enumerate(cfgs["A"]):
        if v == "M":
            ...
            print("tt_x", tt_x.shape, tt_x.layout)
            tt_x = ttnn.to_layout(tt_x, ttnn.ROW_MAJOR_LAYOUT)
            ttact_d = ttnn.to_device(tt_x, device)
            print("MAXPOOL2d", ttact_d.shape, ttact_d.layout)
            print("----------==========--------")
            tt_x = ttnn.max_pool2d(
                input_tensor=ttact_d,
                batch_size=batch_size,
            ...
            print("weights shape", tt_weight.shape, tt_weight.layout)
            tt_weight = ttnn.from_device(tt_weight)
            print("tt_weight", tt_weight.memory_config)
            tt_weight = ttnn.to_layout(tt_weight, layout=ttnn.ROW_MAJOR_LAYOUT)
            tt_bias = parameters.features[conv_feature_ids_2[iter_conv_id]].bias

            # Call ttnn.conv
            conv_op_cache = {}
            print("CONV2d", tt_x.shape, tt_x.layout)
            print("----------==========--------")
            [tt_output_tensor_on_device, out_height, out_width, weights_device, bias_device] = ttnn.conv2d(
                input_tensor=tt_x,
                weight_tensor=tt_weight,
