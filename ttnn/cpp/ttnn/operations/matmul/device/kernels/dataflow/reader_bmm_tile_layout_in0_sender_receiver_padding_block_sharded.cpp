...
void kernel_main() {
    for (uint32_t i = 0; i < 2; ++i) {
        ...
        tt_l1_ptr uint32_t* in0_mcast_noc_x =
            (tt_l1_ptr uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_x)));
        tt_l1_ptr uint32_t* in0_mcast_noc_y =
            (tt_l1_ptr uint32_t*)(get_arg_addr(increment_arg_idx(rt_args_idx, num_y)));
        ...
        constexpr uint32_t num_remote_senders =
            (num_blocks_inner_dim + num_blocks_per_shard - 1) / num_blocks_per_shard;
            ...
            for (uint32_t bh = 0; bh < num_blocks_h_dim; ++bh) {
                for (uint32_t bw = 0; bw < num_blocks_w_dim; ++bw) {
                    ...
                    for (uint32_t block = 0; block < num_blocks_inner_dim; ++block) {
                        ...
                        if (block_id == sender_id) {
                            ...
                            if constexpr (core_in_in0_receiver_mcast_grid) {
                                ...
                                if constexpr (in0_mcast_num_cores == 1) {
                                    ...
                                    in0_mcast_receiver_semaphore_addr_ptr[0] =
                                        in0_mcast_sender_semaphore_valid_addr_ptr[0];
                                } else {
                                ...
                                // If we are not part of receiver grid, always do a regular noc_async_write_multicast to
                                // all cores in receiver grid
                                noc_async_write_multicast(
                                    in0_tensor_read_addr,
                                    in0_multicast_data_addr,
                                    in0_block_size_bytes,
                        ...
                        if constexpr (core_in_in0_receiver_mcast_grid) {
                            // wait on in0 semaphore value to become VALID (set by mcast sender after it multicasts
                            // data)
                        ...
                        // properly for cores that only send and have no compute / writer active. Technically, don't
                        // have to do this if cb_id_in0 is not double buffered.
