...
std::vector<Tensor> ExecuteUnaryBackwardI0::invoke(const Tensor& grad, const Tensor& input, const std::optional<MemoryConfig>& output_mem_config) {
    ...
    Tensor i1_input = ttnn::sign(input, output_mem_config);
    i1_input = ttnn::where(ttnn::eqz(input, output_mem_config), 0.0f, i1_input, output_mem_config);

    Tensor result =  ttnn::multiply(grad, i1_input, std::nullopt, output_mem_config);

    // Tensor value = ttnn::multiply(
    //     ttnn::multiply(i0_input, handled_recip, std::nullopt, output_mem_config),
    //     0.5,
    //     std::nullopt,
    //     output_mem_config);
    // Tensor result = ttnn::where(
    ...
    //     ttnn::multiply(grad,
    //         ttnn::subtract(ttnn::neg(i0_input, output_mem_config), value, std::nullopt, output_mem_config),
    //         std::nullopt,
    //         output_mem_config),
    //     ttnn::multiply(grad,
    //         ttnn::subtract(i0_input, value, std::nullopt, output_mem_config),
    //         std::nullopt,
    //         output_mem_config),
    //     output_mem_config);

    // result =
    //     ttnn::where(ttnn::eqz(input, output_mem_config), 0.0f, result, output_mem_config);

    // result = ttnn::where(
    //     ttnn::ge(ttnn::abs(i0_input, output_mem_config), 3.4e+38, std::nullopt, output_mem_config),
    //     t_inf,
    //     result,
    //     output_mem_config);
    // result =
