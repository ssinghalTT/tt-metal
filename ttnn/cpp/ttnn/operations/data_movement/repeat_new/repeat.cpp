...
repeat_upper_dims_rm(
    ...
    std::cout << " VIEW SHAPE " << ttnn::SimpleShape(collapsed_shape_vector) << std::endl;
    // use ttnn::view to check logic
    auto input_tensor = ttnn::view(tensor, ttnn::SimpleShape(collapsed_shape_vector));

    std::cout << "ADJUSTED SHAPE " << input_tensor.get_logical_shape() << std::endl;
    ...
    auto out_tensor =
        operation::run(RM_REPEAT_STRUCT{repetitions, is_final_dim, output_mem_config}, {input_tensor}, {}, {}, queue_id)
            .at(0);
    std::cout << " OUTPUT SHAPE " << out_tensor.get_logical_shape() << std::endl;
