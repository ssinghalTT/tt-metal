...
Alignment legacyShapeToAlignment(const ttnn::Shape& shape, const PageConfig& page_config, const MemoryConfig& memory_config) {
...
{
    TT_FATAL(alignment_.size() <= 2 || !memory_config_.is_sharded(), "Tensor must be interleaved if alignment has rank greater than 2!");
...
std::optional<ShardSpecBuffer> TensorLayout::compute_shard_spec_buffer(const ttnn::SimpleShape& shape) const {
    ...
    switch (shard_spec.mode) {
        ...
        default:
            TT_THROW("Unsupported shard mode {} in compute_shard_spec_buffer!", shard_spec.mode);
    }
...
Size TensorLayout::compute_physical_shard_shape(const Size& logical_shard_shape) const {
    TT_FATAL(memory_config_.shard_spec.has_value() and memory_config_.shard_spec.value().mode == ShardMode::LOGICAL, "TensorLayout must be logically sharded for compute_physical_shard_shape!");
    const auto& shard_spec = memory_config_.shard_spec.value();
    // TODO: If physical_shard_shape is provided, alignment_ == physical_shard_shape is guaranteed (should we store physical_shard_shape instead?)
    if (shard_spec.physical_shard_shape.has_value()) {
        const auto& physical_shard_shape = shard_spec.physical_shard_shape.value();
        TT_FATAL(physical_shard_shape[0] == alignment_[-2] and physical_shard_shape[1] == alignment_[-1], "Alignment {} must be same as physical shard shape {} provided in shard spec!", alignment_, physical_shard_shape);
...
Size TensorLayout::compute_physical_shape(const ttnn::SimpleShape& shape) const {
    ...
    if (memory_config_.shard_spec.has_value() and memory_config_.shard_spec.value().mode == ShardMode::LOGICAL) {
        ...
        auto get_physical_size = [](auto original_size, auto logical_shard_size, auto physical_shard_size, auto alignment) -> uint32_t {
            ...
            // If we pad all shards except last shard up to physical size and last one only up to nearest alignment, then return this:
            // NOTE: This matches existing physical sharding where physical host data can be sharded with partial shards
            auto num_full_shards = original_size / logical_shard_size;
            auto last_physical_shard_size = CMAKE_UNIQUE_NAMESPACE::round_up(original_size % logical_shard_size, alignment);
            return (physical_shard_size * num_full_shards + last_physical_shard_size);
        };

        auto physical_height = get_physical_size(height, logical_shard_shape.height(), physical_shard_shape.height(), alignment_[-2]);
        auto physical_width = get_physical_size(width, logical_shard_shape.width(), physical_shard_shape.width(), alignment_[-1]);
...
Size TensorLayout::compute_page_shape(const Size& physical_size) const {
    ...
    if (memory_config_.shard_spec.has_value()) {
        ...
        switch (shard_spec.mode) {
            case ShardMode::PHYSICAL:
                physical_shard_shape = shard_spec.shape;
                break;
            case ShardMode::LOGICAL:
                physical_shard_shape = compute_physical_shard_shape(shard_spec.shape);
                break;
            default:
                TT_THROW("Unsupported shard mode {} in compute_shard_spec_buffer!", shard_spec.mode);
        }
