...
void kernel_main() {
    ...
    constexpr uint32_t reader_id = get_compile_time_arg_val(6);
    constexpr uint32_t bf16_scalar_u32 = get_compile_time_arg_val(7);
    constexpr uint32_t in_nblocks_c = get_compile_time_arg_val(8);

    constexpr uint32_t TILE_WIDTH = 32;
    constexpr uint32_t in_cb_id = tt::CB::c_in0;
    constexpr uint32_t in_shard_cb_id = tt::CB::c_in2;
    ...
    constexpr uint32_t in_w_padded = in_w + 2 * pad_w;
    constexpr uint32_t npages_to_reserve = 1;
    uint32_t counter = reader_id;
    while (counter < reader_nindices) {
        // uint16_t top_left_local_index = reader_indices_ptr[counter++];
        uint32_t top_left_local_index = counter++;
            ...
            for (uint32_t h = 0; h < window_h; ++h) {
                for (uint32_t w = 0; w < window_w; ++w) {
                    ...
                    uint32_t read_offset =
                        in_l1_read_base_addr + (stick_offset * in_nbytes_c + c_i * TILE_WIDTH * 8 * 2);
