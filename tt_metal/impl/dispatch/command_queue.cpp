...
void HWCommandQueue::enqueue_write_buffer(
    ...
    std::visit(
        [this, &buffer, &blocking, &sub_device_ids](auto&& data) {
            using T = std::decay_t<decltype(data)>;
            std::visit(
                [this, &buffer, &blocking, &data, &sub_device_ids](auto&& b) {
                    ...
                    if constexpr (std::is_same_v<T, const void*>) {
                        if constexpr (std::is_same_v<type_buf, std::shared_ptr<Buffer>>) {
                            ...
                            this->enqueue_write_buffer(b.get(), data, blocking, sub_device_ids);
                        }
                    } else {
                        if constexpr (std::is_same_v<type_buf, std::shared_ptr<Buffer>>) {
                            this->enqueue_write_buffer(*b, data.get()->data(), blocking, sub_device_ids);
                        } else if constexpr (std::is_same_v<type_buf, std::reference_wrapper<Buffer>>) {
                            this->enqueue_write_buffer(b.get(), data.get()->data(), blocking, sub_device_ids);
                        }
                    }
                },
                buffer);
...
void HWCommandQueue::enqueue_trace(const uint32_t trace_id, bool blocking) {
    ...
    for (const auto& [index, desc]: trace_inst->desc->descriptors) {
        ...
        this->manager.get_config_buffer_mgr(index).mark_completely_full(this->expected_num_workers_completed[index]);
    }
    if (blocking) {
...
void EnqueueReadBufferImpl(
    ...
    tt::stl::Span<const uint32_t> sub_device_ids) {
    std::visit(
        [&cq, dst, blocking, &sub_device_ids](auto&& b) {
            using T = std::decay_t<decltype(b)>;
            if constexpr (
