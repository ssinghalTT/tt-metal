...
int main(int argc, char** argv) {
    ...
    std::string max_kg = std::to_string(std::get<0>(core_count));
    benchmark::RegisterBenchmark(
        "BM_pgm_dispatch/kernel_groups_4_shadow", BM_pgm_dispatch, "-w 5000 -ac -kg " + max_kg + " -rs 40000 -nf 4")
        ->Apply(Max8192Args)
        ->UseManualTime();
    benchmark::RegisterBenchmark(
        "BM_pgm_dispatch/kernel_groups_5_shadow", BM_pgm_dispatch, "-w 5000 -ac -kg " + max_kg + " -rs 40000 -nf 5")
        ->Apply(Max8192Args)
        ->UseManualTime();
    if (getenv("ARCH_NAME") == std::string("wormhole_b0")) {
        benchmark::RegisterBenchmark("BM_pgm_dispatch/eth_dispatch", BM_pgm_dispatch, "-w 5000 -b -n -t +e")
            ...
            ->UseManualTime();
        benchmark::RegisterBenchmark(
            "BM_pgm_dispatch/tensix_eth_2", BM_pgm_dispatch, "-w 5000 -ac -kg " + max_kg + " -+e -a 16")
            ->Apply(Max8192Args)
            ->UseManualTime();
        benchmark::RegisterBenchmark(
            "BM_pgm_dispatch/tensix_eth_2_4_shadow",
            BM_pgm_dispatch,
            "-w 5000 -ac -kg " + max_kg + " -+e -a 16 -rs 40000 -nf 4")
            ->Apply(Max8192Args)
            ->UseManualTime();
    }
