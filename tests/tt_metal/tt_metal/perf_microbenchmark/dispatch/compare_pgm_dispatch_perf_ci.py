...
for name, benchmark in golden_benchmarks.items():
    if name not in result_benchmarks:
        print(f"Golden benchmark {name} missing from results")
        exit_code = 1
        continue
    ...
    if "error_occurred" in benchmark:
        if "error_occurred" not in result:
            ...
            print(f"Error in {name} was fixed in result (with time {result_time:.2f}us). Consider adjusting baselines.")
        continue

    if "error_occurred" in result:
        if "error_occurred" not in benchmark:
            print(f"Benchmark {name} gave unexpected error: {result['error_message']}")
            exit_code = 1
        continue
    ...
    if result_time / golden_time > (1 + THRESHOLD / 100):
        print(f"Test {name} expected value {golden_time:.2f}us but got {result_time:.2f}us")
        exit_code = 1
    if golden_time / result_time > (1 + THRESHOLD / 100):
        print(
            f"Test {name} got value {result_time:.2f}us but expected {golden_time:.2f}us. Consider adjusting baselines"
        )
