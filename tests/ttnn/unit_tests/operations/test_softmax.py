...
def test_softmax(device, batch_size, h, w, dim):
    ...
    torch_input_tensor = torch.arange(0, batch_size * h * w, dtype=torch.bfloat16).reshape(batch_size, h, w)

    # Print the tensor for debugging purposes to verify it contains 0, 1, 2, 3, ...
