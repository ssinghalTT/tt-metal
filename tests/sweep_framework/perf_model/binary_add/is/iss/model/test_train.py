...
def write_to_csv(shard_x, shard_y, start_x, start_y, dims):
    # Open the CSV file in append mode to add the data
    with open(
        "tests/sweep_framework/perf_model/binary_add/is/iss/model/sharding_configurations.csv", mode="a", newline=""
    ) as file:
        writer = csv.writer(file)
        # Write a row with shard_x, shard_y, grid_x, grid_y
        writer.writerow([shard_x, shard_y, start_x, start_y, dims[0], dims[1]])
...
def run_test(
    ...
    write_to_csv(shard_x, shard_y, start_x, start_y, dims)
...
@pytest.mark.parametrize("dims", [1, 3, 8, 14])
@pytest.mark.parametrize("out_mem_config", [ttnn.DRAM_MEMORY_CONFIG])
def test_add_with_block_sharding(device, dims, out_mem_config):
    torch.manual_seed(0)
    ...
    for h_cores in range(1, 9):
        for w_cores in range(1, 9):
            ...
            #     continue
            h = h_cores * dims * 32
            w = w_cores * dims * 32  # 400 tiles per core ~0.8MB
